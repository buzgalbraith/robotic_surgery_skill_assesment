...
class Self_Supervised_Model(Base_Model):
    def __init__(self, input_size, hidden_size, num_layers=2, dropout=0.2, bidirectional=True, kernel_size=3, num_states=10, resolution=2, alpha=0.2):
    ...
    def get_pseudo_labels(self, target_dataset, T=1000, test_steps=None):
        ...
        # last_label = pseudo_labels[-1].reshape(-1,1)
        # try:
        #     pseudo_labels = torch.stack(pseudo_labels[:-1]).reshape(-1,1)
        # except:
        for i in range(len(pseudo_labels)):
            pseudo_labels[i] = torch.tensor(pseudo_labels[i]).reshape(-1,1)
                ## want to add one d vectors
    ...
    def update_datasets(self, self_supervised_dataset, pseudo_labels,indices, K=5):
        ...
        # self_supervised_dataset.update_train_dataset()
    ...
    def target_loss(self, target_dataset, optimizer, criterion, device, epoch, total_epochs):
        ...
        for i, (input_bath, target_bath) in enumerate(target_dataset):
            ...
            for x,y in zip(input_bath, target_bath):
                ...
                final_output = self(x).squeeze()
                loss = criterion(final_output, y) 
                loss.item= loss.item() * self.alpha
            loss.backward()
            optimizer.step()
    ...
    def model_step(self, dataset, optimizer,criterion, device, epoch, total_epochs):
        self.train()
        ## shuffle datasets
        dataset.train_dataset[0].shuffle() ## shuffle the training dataset
        ...
        epoch_loss = source_loss + target_loss
    def self_supervised_train_step(self, self_supervised_dataset, optimizer,criterion, device, epoch, total_epochs, T=1000):
        ...
        ## update datasets
        self_supervised_dataset = self.update_datasets(self_supervised_dataset, pseudo_labels,indices)
        ## train on updated source dataset.
        self.model_step(self_supervised_dataset, optimizer, criterion, device, epoch, total_epochs)
